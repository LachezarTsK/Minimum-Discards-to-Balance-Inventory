...
function minArrivalsToDiscard(itemsArrival: number[], windowSize: number, maxNumberSameItemsInWindow: number): number {
    const REMOVED_ITEM = -1;
    const maxItemValue = Math.max(...itemsArrival);
    let numberOfDiscards = 0;
    const frequencyItems: number[] = new Array(maxItemValue + 1).fill(0);
